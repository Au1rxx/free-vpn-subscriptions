...
- [ ] **Task 4: 版本化低并发首验 oneshot**
  ...
  - [ ] **Step 1: 先扩展契约测试并确认 RED**

    在 required 数组增加模板；断言 `Type=oneshot`、`RuntimeMaxSec=30min`、credential、`--once --initial-protocol=%i --limit 100 --concurrency 20`、`validator-id=ai-a1-initial-%i`。运行测试，Expected: 因 unit 不存在而 FAIL。

  - [ ] **Step 2: 创建受限 unit 并加入安装器**

    unit 复用主 Worker 的 tunnel 依赖、安全沙箱和状态目录，但设置 `Type=oneshot`、`RuntimeMaxSec=30min`、`CPUQuota=100%`、`MemoryMax=2G`、`TasksMax=512`，不设置 Restart。安装器的 systemd 文件集合必须包括 `free-vpn-validate-initial@.service`。

  - [ ] **Step 3: 运行 GREEN 并提交私有仓库变更**
...
- [ ] **Task 5: 补测 HTTP/HTTPS/SOCKS5 并证明全链路无回归**
  - Status: pending
...
- 本计划只有 5 个顶层任务全部验证并通过代码审阅后才可移入 completed 状态。
- 总平台目标仍要求 72 小时影子硬指标；本计划完成不得被用作发布切换依据。
