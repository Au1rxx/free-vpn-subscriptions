...
var KO = Locale{
	...
	Hook2:       "가입 불필요. 결제 불필요. 바이너리 설치 불필요. 공개 소스에서 매시간 자동 갱신 — 발행 전 모든 노드를 TCP + TLS 로 검증.",
	...
	VerificationBody: `**솔직히 말하면: 어떤 노드가 반드시 트래픽을 통과시킨다고 *보장* 할 수는 없습니다.** 실제로 트래픽을 흘려보내지 않는 한, 어떤 집계 프로젝트도 불가능합니다. 아래에 "집계 단계에서 무엇을 검증하고, 무엇을 못 하며, 진짜 보장은 어디에서 오는지"를 모두 밝힙니다.
...
발행되는 상위 200 개 노드 중, 클라이언트는 보통 30-50 개의 실제로 HTTP 를 통과시키는 노드를 찾아냅니다. 느려지면 url-test 그룹이 다음 후보로 전환하여 한 번의 클릭이면 충분합니다.`,
	...
	FAQ2A:      "매시간 갱신 (상위 소스를 `:00` 에 집중적으로 때리지 않도록 작은 무작위 지연 포함): 모든 상위 소스 가져오기 → 각 노드 TCP+TLS 프로브 → 죽은 것 제거 → 레이턴시 순 정렬 → 새 출력 파일 발행. 위의 `Last updated` 타임스탬프를 확인하세요.",
	...
	FAQ4A:      "TCP 도달성과 TLS 핸드셰이크만 검증하므로 노드는 여전히 할당량 소진, 잘못된 라우팅, 만료된 인증서를 가질 수 있습니다. 발행하는 `clash.yaml` 에는 `url-test` 그룹 (`http://www.gstatic.com/generate_204`, 300초 간격) 이 포함되어 있어 클라이언트가 실제로 HTTP 를 통과시키는 가장 빠른 노드를 자동 선택합니다. 죽으면 다음으로.",
