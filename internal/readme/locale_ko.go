...
var KO = Locale{
	...
	Hook2:       "가입 불필요. 결제 불필요. 바이너리 설치 불필요. 공개 소스에서 매시간 자동 갱신 —— 발행되는 모든 노드는 몇 분 전에 sing-box 를 통해 실제 HTTP 트래픽을 전달한 이력이 있습니다.",
	KeywordLine: "무료 VPN 구독 · 무료 v2ray 구독 · 무료 Clash 구독 · 무료 sing-box 구독 · VLESS · Reality · VMess · Trojan · Shadowsocks · Hysteria2 · 매시간 갱신 · HTTP 실측 검증 · 국가별",

	WhyHeading: "## 💡 왜 이 프로젝트?",
	WhyBody:    "GitHub의 거의 모든 \"무료 VPN\" 목록은 데이터가 오래되었거나, 죽은 노드로 가득 차 있거나, 출처가 불분명한 바이너리 설치를 요구합니다. 이 저장소는 다른 어떤 곳보다 한 단계 더 나아갑니다 —— **단지 포트가 열려 있는지 확인하는 것이 아니라, sing-box 로 실제 HTTP 트래픽을 노드를 거쳐 보내고 204 가 돌아오는지 확인한 뒤에만 발행합니다**, 모두 수 분 내에. Clash / sing-box / v2rayN에 바로 붙여 넣을 수 있는 3가지 범용 구독 파일을 제공합니다.",

	VerificationHeading: "## 🔬 노드가 실제로 작동하는지 어떻게 검증하나",
	VerificationBody: `대부분의 무료 VPN 목록은 "TCP 포트가 열려 있다"에서 멈추고 발행합니다. 우리는 다릅니다. 아래는 노드가 구독에 포함되기 전에 통과해야 하는 전체 검증 파이프라인입니다.
...
발행되는 ~150 개 노드 중, 클라이언트는 보통 **80-120 개가 당신의 네트워크에서 HTTP 를 통과시키는 것을** 찾아냅니다 —— TCP 프로브만 하는 목록보다 2-3 배 높은 명중률입니다. 하나가 느려지면 url-test 그룹이 투명하게 로테이션합니다.`,
	...
	FAQ2A:      "매시간 갱신 (상위 소스를 `:00` 에 집중적으로 때리지 않도록 작은 무작위 지연 포함): 모든 소스 가져오기 → TCP → TLS → sing-box 설정 검증 → HTTP-over-proxy 프로브 (2 라운드, 45 초 간격) → 실 HTTP 레이턴시 정렬 → 새 출력 파일 발행. 전체 파이프라인 약 10 분. 위의 `Last updated` 타임스탬프를 확인하세요.",
	...
	FAQ4A:      "HTTP-over-proxy 프로브를 통과한 후에도 노드는 집계 사이에 죽을 수 있습니다: 할당량 소진, 상위 키 취소, ISP 가 출구 IP 차단, 운영자 중단 등. 발행하는 `clash.yaml` 에는 `url-test` 그룹 (`http://www.gstatic.com/generate_204`, 300초 간격) 이 포함되어 있어 클라이언트가 *당신의 네트워크에서* 실제로 HTTP 를 통과시키는 가장 빠른 노드를 자동 선택합니다. 죽으면 다음으로. 150 개 중 80-120 개가 수시로 작동합니다.",
	...
	StarCTA:            "⭐ **Star를 눌러 주세요** — 시간을 아끼셨다면, 다음에 무엇을 만들지 결정하는 유일한 신호가 됩니다.",
