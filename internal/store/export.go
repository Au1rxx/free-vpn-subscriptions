...
func ListExportable(ctx context.Context, db *sql.DB, query ExportQuery) ([]*node.Node, []ExportMeta, error) {
	...
	rows, err := db.QueryContext(ctx, `SELECT n.node_config_id, n.normalized_config, n.config_fingerprint,
		s.quality_grade, s.quality_score, COALESCE(s.exit_country,''), COALESCE(s.latency_p50_ms,0)
	...
	for rows.Next() {
		...
		var country string
		var latency int
		if err := rows.Scan(&meta.ConfigID, &body, &fingerprint, &meta.Grade, &meta.Score, &country, &latency); err != nil {
			return nil, nil, err
		}
